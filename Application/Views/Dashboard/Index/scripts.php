...
//            dashboard.jsTreeInstance.init(dashboard.currentProject);
        ...
        .jstree({
            'core': {
                'data': {
                    'url': 'index.php?controller=dashboard&action=projectattachementsapi&salesorder=10005&operation=get_node',
                    'data': function (node) {
                        return {'id': node.id};
                    }
                },
            ...
            'types': {
                '#': {'max_children': 1, 'valid_children': ['default'], 'icon': 'glyphicon glyphicon-folder-close'},
                'default': {'icon': 'glyphicon glyphicon-folder-close'},
                'file': {'valid_children': [], 'icon': 'glyphicon glyphicon-folder-close'}
            },
            'unique': {
