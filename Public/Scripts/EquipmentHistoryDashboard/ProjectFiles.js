/**
 * @author Alex
 * @namespace App.EquipmentHistoryDashboard.ProjectFiles
 * @param {type} global
 * @param {type} $
 * @param {type} App
 * @returns {undefined}
...
(function (global, $, App) {
    ...
    var dandelion = global.dandelion,
        ProjectFiles = dandelion.namespace('App.EquipmentHistoryDashboard.ProjectFiles', global);
            ...
            init: function () {
                this.on('removedfile', function (file) {
                    var selectedDir = ProjectFiles.status.jsTree.jstree(true).get_selected(),
                        params = {
                            rootDir: App.EquipmentHistoryDashboard.getCurrentID(),
                            selectedDir: selectedDir[0],
                            fileName : file.name
                        };
                ...
                this.on('sending', function (file, xhr, formData) {
                    ...
                    file.ready4Remove = true;
                    file.uploadPath = App.EquipmentHistoryDashboard.getCurrentID() + '/' + selectedDir;
                    if (App.EquipmentHistoryDashboard.getCurrentID()) {
                        formData.append('rootDir', App.EquipmentHistoryDashboard.getCurrentID());
