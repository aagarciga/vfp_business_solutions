/**
 * @author Alex
 * @namespace App.APDashboard.DynamicFilter
 * @param {window} global
 * @param {jQuery} $
 * @param {Object} App
 * @returns {undefined}
...
(function (global, $, App) {
    ...
    var dandelion = global.dandelion,
        DynamicFilter = dandelion.namespace('App.APDashboard.DynamicFilter', global);
    ...
    DynamicFilter.functions.filter = function () {
        App.APDashboard.functions.paginate();
    };
    DynamicFilter.functions.loadFilter = function (filterId) {
        $.ajax({
            ...
            url: App.APDashboard.urls.getSavedFilter,
            type: 'post',
            beforeSend: function () {
                $('.loading').show();
            },
    ...
    DynamicFilter.functions.createDropdownField = function (field, caption, options, valueType) {
        var $formGroup = $('<div class="form-group" title="' + caption +
            ...
            $select = $formGroup.find('select'), index, current;
    ...
    DynamicFilter.functions.createDropdownSavedFilterItem = function (filterId, caption) {
        var tmpl = '<li><a href="#" class="saved-filter-list-item" data-filterid="' + filterId +
            ...
            $control = $(tmpl);
        $control.children('a')
            .on('click', DynamicFilter.eventHandlers.drpSavedFilterItem_onClick);

        $control.find('button.close')
            .on('click', DynamicFilter.eventHandlers.drpSavedFilterItem_btnDelete_onClick);
        return $control;
    };
    ...
    DynamicFilter.eventHandlers.drpSavedFilterItem_btnDelete_onClick = function (event) {
        ...
        $.ajax({
            ...
            url: App.APDashboard.urls.deleteFilter,
            type: 'post',
            beforeSend: function () {
                $('.loading').show();
            },
    ...
    DynamicFilter.eventHandlers.filterFields_btnAdd_onClick = function (event) {
        ...
        if (fieldType === 'text') {
            ...
            dropdownValues = global.App.APDashboard.dictionaries[$button.data('field-collection')];
    ...
    DynamicFilter.eventHandlers.modalSaveFilter_btnSave_onClick = function () {
        ...
        if (filterName !== "" && /\w/.test(filterName)) {
            $.ajax({
                ...
                url: App.APDashboard.urls.saveFilter,
                type: 'post',
                beforeSend: function () {
                    $('.loading').show();
                },
